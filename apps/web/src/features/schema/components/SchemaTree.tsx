...
function ConnectionBlock({
    ...
    const { data: schemas, isLoading: loadingSchemas, isError, error } = useSchemas(isExpanded ? connection.id : undefined)
    ...
    const errorMessage = error instanceof Error
        ? error.message
        : (error as any)?.message || 'Failed to connect to database'
    ...
    return (
        <div className="connection-block">
            {/* Connection Header */}
            <div className={`connection-header ${isExpanded ? 'expanded' : ''} ${isError ? 'has-error' : ''}`} onClick={onToggle}>
                ...
                {isError && (
                    <span className="connection-error-indicator" title={errorMessage}>⚠</span>
                )}
                {!isError && connection.isActive && (
                    <span className="connection-status">✓</span>
                )}

                {/* Three-dot menu */}
                <div className="connection-menu-wrapper" ref={menuRef}>
                    <button
                    ...
                    {showMenu && (
                        <div className="connection-menu-dropdown">
                            <button className="connection-menu-item" onClick={handleEdit}>
                                <FaEdit size={12} />
                                <span>Edit Connection</span>
                            </button>
                            <button className="connection-menu-item danger" onClick={handleDelete}>
                                <FaTrash size={12} />
                    ...
                    {loadingSchemas ? (
                        ...
                        <div className="schema-status error">
                            ...
                            <div className="error-content">
                                <span className="error-title">Connection failed</span>
                        ...
                        <div className="schema-status empty">
